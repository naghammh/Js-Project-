...
// تفعيل الصورة التالية والسابقة عند النقر على الأسهم المخصصة
// ربط الأسهم المخصصة
document.querySelector('.custom-next').addEventListener('click', () => {
  swiper.slideNext(); // صورة لليمين
});

document.querySelector('.custom-prev').addEventListener('click', () => {
  swiper.slidePrev(); // صورة لليسار
});
...
// تنسيق نموذج البحث
  ...
  // اختياري: عند الضغط على الزر يرجع لأعلى الصفحة
